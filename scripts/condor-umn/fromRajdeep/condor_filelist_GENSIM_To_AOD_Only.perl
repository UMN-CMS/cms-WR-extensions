...
print(SUBMIT "request_memory = 3350\n");
print(SUBMIT "Requirements = (Arch==\"X86_64\")");
# Zebras are for remote login, not cluster computing
print(SUBMIT " && (Machine != \"zebra01.spa.umn.edu\" && Machine != \"zebra02.spa.umn.edu\" && Machine != \"zebra03.spa.umn.edu\" && Machine != \"zebra04.spa.umn.edu\" && Machine != \"caffeine.spa.umn.edu\" && Machine != \"scorpion6.spa.umn.edu\")");
...
sub specializeCfg($$@) {
	...
	if (/ALCARECOStreamEcalUncalZElectron/) {
	    $sector=4;
	    $had3=1;
	}    
        if (/[.]Source/) {
        ...
        } elsif ($sector==4 && /^[^\#]*fileName\s*=/) {
	    if ($had2==1) {
	        $fname="$prodSpace/$jobBase/".$stub2."-pool.root";
	    } else {
	        $fname="$prodSpace/$jobBase/EcalUncalZElectron_".$stub2.".root";
	    } 	
	    unlink($fname);
	    print OUTP "       fileName = cms.untracked.string(\"$fname\"),\n";
