...
def SignalRegionCard(workspaceDirectory, year):
    ...
    bkgs = ['TT','ST','WJets','DY','DiBoson','TriBoson','ttV']
    Allbkgs = ['TT','ST','WJets','DY','DiBoson','TriBoson','ttV']
...
#    sigs = ['2000,1000','3000,1500','4000,2000']
	...
	muIDString = 'MuID_%s\tshapeN2'%(year)
...
#        datacard+=lumiString+jecString+jerString+puString+muResolString+ZweightString+ScaleUncString+PDFUncString+AlphaSUncString
        datacard+=lumiString+jecString+jerString+puString+muIDString+muResolString+ScaleUncString+PDFUncString+AlphaSUncString+ZweightString
#    	datacard+=lumiString+jecString+jerString+puString+muIDString+muResolString+muTrigString+ZweightString+ScaleUncString+PDFUncString+AlphaSUncString

	datacard+='DYNorm_%s lnN - - - - 1.03 - - -\n'%(year)
#	datacard+='Syst_%s lnN - - - - - - - 1.2\n'%(year)
        datacard+='R_Resolved_MuMu_%s rateParam ResolvedSignalRegion%s TT 1\n'%(year,year)
#        'DYNorm rateParam ResolvedSignalRegion DY 1.069 [1.061,1.077]\n' + \
        datacard+='* autoMCStats 0\n'

    	dctmp.write(datacard)

def FSBCard(workspaceDirectory, year):
    if year == "2016":
        workspace = workspaceDirectory+'WR_NR_MuMuJJ_ResolvedFSB2016.root'
    elif year == "2017":
        workspace = workspaceDirectory+'WR_NR_MuMuJJ_ResolvedFSB2017.root'
    elif year == "2018":
        workspace = workspaceDirectory+'WR_NR_MuMuJJ_ResolvedFSB2018.root'
...
#    bkgs = ['TT','ST','WJets','DY','DiBoson','QCD']
    ...
    for WRmass,NRmass in signalMasses:
        ...
        datacard = 'imax 1 number of channels\n' + \
            ...
            'kmax * number of nuisance parameters\n' + \
            divider + \
           'shapes * ResolvedFSB%s WR_NR_MuMuJJ_ResolvedFSB%s.root MuMuJJ/$PROCESS MuMuJJ/$PROCESS_$SYSTEMATIC\n'%(year,year) + \
            divider + \
            'bin             ResolvedFSB%s\n'%(year) + \
            'observation     -1.0\n' + \
            divider
        ...
        jerString = 'JER_%s\tshapeN2'%(year)
        puString = 'PU_%s\tshapeN2'%(year)
        HEEPString = 'HEEP_%s\tshapeN2'%(year)
        muIDString = 'MuID_%s\tshapeN2'%(year)
        muResolString = 'MuResol_%s\tshapeN2'%(year)
        muTrigString = 'MuTrig_%s\tshapeN2'%(year)
        ZweightString = 'Zweight_%s\tshapeN2'%(year)
...
#           print "histoDict['%s'%(bkg)].Integral(): ", histoDict['%s'%(bkg)].Integral()
        ...
        datacard+=lumiString+jecString+jerString+puString+HEEPString+muIDString+muResolString+ZweightString
        if histoDict['WR_%s_NR_%s'%(WRmass,NRmass)].Integral() > 0:
            datacard+='DYNorm_%s lnN - - - - 1.03 - - -\n'%(year)
	else:
            datacard+='DYNorm_%s lnN - - - 1.03 - - -\n'%(year)
        datacard+='R_Resolved_MuMu_%s rateParam ResolvedFSB%s TT 1\n'%(year,year)
        datacard+='* autoMCStats 0\n'
...
FSBCard(workspaceDirectory, year)
