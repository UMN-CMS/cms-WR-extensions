	...
	systs = ['JEC','JER','PU','MuResID','MuResol','MuTrig']
        ...
        if 'TTTo' in sample:
          if 'TTToSemi' in sample:
            ...
            temp = tfile.Get('analysis/eventsPassResFailBoostRECO/resolvedRECOmass').Rebin(Nbins, 'TT', binBoundariesArray)
            histoDict['TT'].Add(temp,weights[sample])

            for syst in systs:
               temp = tfile.Get('analysis/eventsPassResFailBoostRECO/resolvedRECOmass_%sUp'%(syst)).Rebin(Nbins, 'TT_temp', binBoundariesArray)
               histoDict['TT_%sUp'%(syst)].Add(temp,weights[sample])
               temp = tfile.Get('analysis/eventsPassResFailBoostRECO/resolvedRECOmass_%sDown'%(syst)).Rebin(Nbins, 'TT_temp', binBoundariesArray)
		...
		temp = tfile.Get('analysis/eventsPassResFailBoostRECO/resolvedRECOmass').Rebin(Nbins, 'DY_temp', binBoundariesArray)
		    ...
		    temp = tfile.Get('analysis/eventsPassResFailBoostRECO/resolvedRECOmass_%sDown'%(syst)).Rebin(Nbins, 'DY_temp', binBoundariesArray)
                ...
                temp = tfile.Get('analysis/eventsPassResFailBoostRECO/resolvedRECOmass_ZweightUp').Rebin(Nbins, 'DY_temp', binBoundariesArray)
                histoDict['DY_ZweightUp'].Add(temp,weights[sample]*ZPeakSF)
                temp = tfile.Get('analysis/eventsPassResFailBoostRECO/resolvedRECOmass_ZweightDown').Rebin(Nbins, 'DY_temp', binBoundariesArray)
...
#		    print "histoDict['WR_%s_NR_%s_Error_%s'%(wrMass,nuMass,i)].Integral(): ", histoDict['WR_%s_NR_%s_Error_%s'%(wrMass,nuMass,i)].Integral()
            ...
            for j in range(1,histoDict['WR_%s_NR_%s'%(wrMass,nuMass)].GetNbinsX()+1):
                ...
                if histoDict['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].GetBinContent(j) == 0:
                    ...
                    print "math.sqrt(SquaredBinError[j])/histoDict['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].GetBinContent(j)", math.sqrt(SquaredBinError[j])/histoDict['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].GetBinContent(j)
...
#               lif math.sqrt(SquaredBinError[j])/histoDict['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].GetBinContent(j) < 1:
		    ...
		    print "nominal PDF case"
                    print "math.sqrt(SquaredBinError[j])/histoDict['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].GetBinContent(j)", math.sqrt(SquaredBinError[j])/histoDict['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].GetBinContent(j)
		    print "histoDict['WR_%s_NR_%s'%(wrMass,nuMass)].GetBinContent(j)*(1 +  math.sqrt(SquaredBinError[j])/histoDict['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].GetBinContent(j)): ", histoDict['WR_%s_NR_%s'%(wrMass,nuMass)].GetBinContent(j)*(1 +  math.sqrt(SquaredBinError[j])/histoDict['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].GetBinContent(j))
		    print "histoDict['WR_%s_NR_%s'%(wrMass,nuMass)].GetBinContent(j)*(1 -  math.sqrt(SquaredBinError[j])/histoDict['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].GetBinContent(j)): ", histoDict['WR_%s_NR_%s'%(wrMass,nuMass)].GetBinContent(j)*(1 -  math.sqrt(SquaredBinError[j])/histoDict['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].GetBinContent(j))
                    histoDict['WR_%s_NR_%s_PDFUp'%(wrMass,nuMass)].SetBinContent(j, histoDict['WR_%s_NR_%s'%(wrMass,nuMass)].GetBinContent(j)*(1 +  math.sqrt(SquaredBinError[j])/histoDict['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].GetBinContent(j)))
...
#    histoDict['data_obs'].Write()
    for syst in systs:
	if syst == "MuResID":
	     histoDict['TT_%sUp'%(syst)].SetName("TT_MuID_%sUp"%(year))
             histoDict['TT_%sDown'%(syst)].SetName("TT_MuID_%sDown"%(year))
             histoDict['TriBoson_%sUp'%(syst)].SetName("TriBoson_MuID_%sUp"%(year))
             histoDict['TriBoson_%sDown'%(syst)].SetName("TriBoson_MuID_%sDown"%(year))
             histoDict['DiBoson_%sUp'%(syst)].SetName("DiBoson_MuID_%sUp"%(year))
             histoDict['DiBoson_%sDown'%(syst)].SetName("DiBoson_MuID_%sDown"%(year))
             histoDict['ttV_%sUp'%(syst)].SetName("ttV_MuID_%sUp"%(year))
             histoDict['ttV_%sDown'%(syst)].SetName("ttV_MuID_%sDown"%(year))
             histoDict['ST_%sUp'%(syst)].SetName("ST_MuID_%sUp"%(year))
             histoDict['ST_%sDown'%(syst)].SetName("ST_MuID_%sDown"%(year))
             histoDict['WJets_%sUp'%(syst)].SetName("WJets_MuID_%sUp"%(year))
             histoDict['WJets_%sDown'%(syst)].SetName("WJets_MuID_%sDown"%(year))
             histoDict['DY_%sUp'%(syst)].SetName("DY_MuID_%sUp"%(year))
             histoDict['DY_%sDown'%(syst)].SetName("DY_MuID_%sDown"%(year))
...
#                    histoDict['WR_%s_NR_%s'%(WRmass,NRmass)].SetBinContent(j,0.0000001)
            histoDict['WR_%s_NR_%s'%(WRmass,NRmass)].Write()
            histoDict['WR_%s_NR_%s_Numerator'%(WRmass,NRmass)].Write()
            for syst in systs:
	        if syst == "MuResID":
		    histoDict['WR_%s_NR_%s_%sUp'%(WRmass,NRmass,syst)].SetName('WR_%s_NR_%s_MuID_%sUp'%(WRmass,NRmass,year))
                    histoDict['WR_%s_NR_%s_%sDown'%(WRmass,NRmass,syst)].SetName('WR_%s_NR_%s_MuID_%sDown'%(WRmass,NRmass,year))
    ...
    for sample in sampleNames:
        ...
        if 'TTTo' in sample:
          if 'TTToSemi' in sample:
            ...
            temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass').Rebin(Nbins, 'TT', binBoundariesArray)
            histoDictFSB['TT'].Add(temp,weights[sample])

            for syst in systs:
               temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_%sUp'%(syst)).Rebin(Nbins, 'TT_temp', binBoundariesArray)
               histoDictFSB['TT_%sUp'%(syst)].Add(temp,weights[sample])
               temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_%sDown'%(syst)).Rebin(Nbins, 'TT_temp', binBoundariesArray)
		...
		temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass')
                ...
                temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass').Rebin(Nbins, 'WJets_temp', binBoundariesArray)
                histoDictFSB['WJets'].Add(temp,weights[sample])
                for syst in systs:
                    temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_%sUp'%(syst)).Rebin(Nbins, 'WJets_temp', binBoundariesArray)
                    histoDictFSB['WJets_%sUp'%(syst)].Add(temp,weights[sample])
                    temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_%sDown'%(syst)).Rebin(Nbins, 'WJets_temp', binBoundariesArray)
            ...
            if 'DYJetsToLL_M-50_HT-100to200' in sample:
                ...
                histoDictFSB['DY'].Scale(weights[sample]*ZPeakSF)
                for syst in systs:
                    temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_%sUp'%(syst))
                    ...
                    histoDictFSB['DY_%sUp'%(syst)].Scale(weights[sample]*ZPeakSF)
                    temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_%sDown'%(syst))
                    ...
                    histoDictFSB['DY_%sDown'%(syst)].Scale(weights[sample]*ZPeakSF)
                temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_ZweightUp')
                ...
                temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_ZweightDown')
		...
		temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass').Rebin(Nbins, 'DY_temp', binBoundariesArray)
		    ...
		    temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_%sUp'%(syst)).Rebin(Nbins, 'DY_temp', binBoundariesArray)
                    histoDictFSB['DY_%sUp'%(syst)].Add(temp,weights[sample]*ZPeakSF)
		    temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_%sDown'%(syst)).Rebin(Nbins, 'DY_temp', binBoundariesArray)
                    histoDictFSB['DY_%sDown'%(syst)].Add(temp,weights[sample]*ZPeakSF)
                temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_ZweightUp').Rebin(Nbins, 'DY_temp', binBoundariesArray)
                histoDictFSB['DY_ZweightUp'].Add(temp,weights[sample]*ZPeakSF)
                temp = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass_ZweightDown').Rebin(Nbins, 'DY_temp', binBoundariesArray)
            ...
            temp2 = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass')
            ...
            if 'SingleMuon--Run2016B' in sample or 'SingleMuon--Run2017B' in sample or 'SingleMuon--Run2018A' in sample:
                temp2 = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass')
                histoDictFSB['data_obs'] = temp2.Rebin(Nbins, 'data_obs', binBoundariesArray)
                histoDictFSB['data_obs'].SetDirectory(0)
            else:
                temp2 = tfile.Get('analysis/eventsPassResFSBRECO/resolvedFSBRECOmass').Rebin(Nbins, 'data_obs_temp', binBoundariesArray)
                histoDictFSB['data_obs'].Add(temp2)


        tfile.Close()

    if year == "2016":
        outputFile = workspaceOutputDirectory+'WR_NR_MuMuJJ_ResolvedFSB2016.root'
    elif year == "2017":
        outputFile = workspaceOutputDirectory+'WR_NR_MuMuJJ_ResolvedFSB2017.root'
    elif year == "2018":
        outputFile = workspaceOutputDirectory+'WR_NR_MuMuJJ_ResolvedFSB2018.root'
