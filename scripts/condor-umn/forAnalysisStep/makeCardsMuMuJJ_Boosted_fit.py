...
def SignalRegionCard(workspaceDirectory):
    ...
    systs = ['JEC','JER','PU','MuID','MuTrig','MuResol','LSF','Zweight']
...
#    sigs = ['2000,1000','3000,1500','4000,2000']
	...
	rateMuIDUp = histoDict['%s_MuIDUp'%(bkg)].Integral()
        rateMuIDDown = histoDict['%s_MuIDDown'%(bkg)].Integral()
	...
	rateLSFUp = histoDict['%s_LSFUp'%(bkg)].Integral()
	rateLSFDown = histoDict['%s_LSFDown'%(bkg)].Integral()
	...
	if rate > 0:
	    ...
	    LSFErrs['%s'%(bkg)] = 1.0+(abs(rateLSFUp-rate)+abs(rateLSFDown-rate))/(2.*rate)
            if bkg == 'DY':
...
#	LSFErrs['%s'%(bkg)] = 1.0
    ...
    for WRmass,NRmass in signalMasses:
        ...
        rateMuIDUp = histoDict['WR_%s_NR_%s_MuIDUp'%(WRmass,NRmass)].Integral()
        rateMuIDDown = histoDict['WR_%s_NR_%s_MuIDDown'%(WRmass,NRmass)].Integral()
        ...
        rateLSFUp = histoDict['WR_%s_NR_%s_LSFUp'%(WRmass,NRmass)].Integral()
        rateLSFDown = histoDict['WR_%s_NR_%s_LSFDown'%(WRmass,NRmass)].Integral()
	...
	print "rateScale: ", rateScale
        ...
        if rate > 0:
            ...
            LSFErrs['WR_%s_NR_%s'%(WRmass,NRmass)] = 1.0+(abs(rateLSFUp-rate)+abs(rateLSFDown-rate))/(2.*rate)
...
#	LSFErrs['WR_%s_NR_%s'%(WRmass,NRmass)] = 1.0
    ...
    for WRmass,NRmass in signalMasses:
    	...
    	datacard = 'imax 1 number of channels\n' + \
            ...
            divider + \
	    'shapes * BoostedSignalRegion WR_NR_MuMuJJ_BoostedSignalRegion.root MuMuJJ/$PROCESS MuMuJJ/$PROCESS_$SYSTEMATIC\n' + \
	    divider + \
	    'bin             BoostedSignalRegion\n' + \
            'observation     -1.0\n' + \
            divider
