...
#	datacard+='Syst_%s lnN - - - - - - - 1.3\n'%(year)
        datacard+='R_Boosted_MuMu_%s rateParam BoostedSignalRegion%s TT 1\n'%(year,year)
...
def FSBCard(workspaceDirectory, year):
    ...
    if year == "2016":
        workspace = workspaceDirectory+'WR_NR_MuMuJJ_BoostedFSB2016.root'
    elif year == "2017":
        workspace = workspaceDirectory+'WR_NR_MuMuJJ_BoostedFSB2017.root'
    elif year == "2018":
        workspace = workspaceDirectory+'WR_NR_MuMuJJ_BoostedFSB2018.root'
...
#    bkgs = ['TT','ST','WJets','DY','DiBoson','QCD']
    ...
    for WRmass,NRmass in signalMasses:
        ...
        ElHLTErrs['WR_%s_NR_%s'%(WRmass,NRmass)] = 1.0
        LSFErrs['WR_%s_NR_%s'%(WRmass,NRmass)] = 1.0
    ...
    for WRmass,NRmass in signalMasses:
        ...
        datacard = 'imax 1 number of channels\n' + \
            ...
            'kmax * number of nuisance parameters\n' + \
            divider + \
            'shapes * BoostedFSB%s WR_NR_MuMuJJ_BoostedFSB%s.root MuMuJJ/$PROCESS MuMuJJ/$PROCESS_$SYSTEMATIC\n'%(year,year) + \
            divider + \
            'bin             BoostedFSB%s\n'%(year) + \
            'observation     -1.0\n' + \
            divider
	...
	ElHLTString ='ElHLT_%s\tshapeN2'%(year)
        LSFString = 'LSF_%s\tshapeN2'%(year)
        PDFUncString = 'PDF\tshapeN2'
        AlphaSUncString = 'AlphaS\tshapeN2'
        ZweightString = 'Zweight_%s\tshapeN2'%(year)
	    ...
	    ElHLTString += '\t%.3f'%ElHLTErrs['WR_%s_NR_%s'%(WRmass,NRmass)]
            ZweightString += '\t-'
            LSFString += '\t%.3f'%LSFErrs['WR_%s_NR_%s'%(WRmass,NRmass)]
            PDFUncString += '\t1'
            AlphaSUncString += '\t1'
	    ...
	    muIDString += '\t%.3f'%muIDErrs['%s'%(bkg)]
            LSFString += '\t%.3f'%LSFErrs['%s'%(bkg)]
	    ElHLTString += '\t%.3f'%ElHLTErrs['%s'%(bkg)]
            PDFUncString += '\t-'
            AlphaSUncString += '\t-'
            if bkg == 'DY':
                ZweightString += '\t%.3f'%ZweightsErrs['%s'%(bkg)]
        ...
        datacard+=lumiString+jecString+jerString+puString+muIDString+HEEPString+ElHLTString+LSFString+ZweightString
        if histoDict['WR_%s_NR_%s'%(WRmass,NRmass)].Integral() > 0:
            datacard+='DYNorm_%s lnN - - - - 1.03 - - - -\n'%(year)
	else:
            datacard+='DYNorm_%s lnN - - - 1.03 - - - -\n'%(year)
        datacard+='R_Boosted_MuMu_%s rateParam BoostedFSB%s TT 1\n'%(year,year)
        datacard+='* autoMCStats 0\n'
