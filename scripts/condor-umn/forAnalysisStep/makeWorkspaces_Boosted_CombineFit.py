        ...
        if 'TTTo' in sample:
          if 'TTToSemi' in sample:
            ...
            for syst in systs:
                ...
                histoDict['TT_%sUp'%(syst)] = temp.Rebin(Nbins, 'TT_%s_%sUp'%(syst,year), binBoundariesArray)
                ...
                histoDict['TT_%sDown'%(syst)] = temp.Rebin(Nbins, 'TT_%s_%sDown'%(syst,year), binBoundariesArray)
                histoDict['TT_%sDown'%(syst)].SetDirectory(0)
                histoDict['TT_%sDown'%(syst)].Scale(weights[sample])

            histoDict['TT_LSFUp'] = histoDict['TT'].Clone("TT_LSF_%sUp"%(year))
            histoDict['TT_LSFUp'].SetDirectory(0)

            histoDict['TT_LSFUp'].Scale(LSFSFUp/LSFSF)
            histoDict['TT_LSFDown'] = histoDict['TT'].Clone("TT_LSF_%sDown"%(year))
            histoDict['TT_LSFDown'].SetDirectory(0)
            histoDict['TT_LSFDown'].Scale(LSFSFDown/LSFSF)

          else:
            temp = tfile.Get('analysis/eventsFailResPassBoostRECO/leadAK8JetMuonMass').Rebin(Nbins, 'TT', binBoundariesArray)
            histoDict['TT'].Add(temp,weights[sample])
            histoDict['TT_LSFUp'].Add(temp,weights[sample]*LSFSFUp/LSFSF)
            histoDict['TT_LSFDown'].Add(temp,weights[sample]*LSFSFDown/LSFSF)

            for syst in systs:
               temp = tfile.Get('analysis/eventsFailResPassBoostRECO/leadAK8JetMuonMass_%sUp'%(syst)).Rebin(Nbins, 'TT_temp', binBoundariesArray)
               histoDict['TT_%sUp'%(syst)].Add(temp,weights[sample])
               temp = tfile.Get('analysis/eventsFailResPassBoostRECO/leadAK8JetMuonMass_%sDown'%(syst)).Rebin(Nbins, 'TT_temp', binBoundariesArray)
               histoDict['TT_%sDown'%(syst)].Add(temp,weights[sample])
...
#    ttbarEst.Scale(137400./35900.)
    ...
    if year == '2016':
        outputFile = workspaceOutputDirectory+'WR_NR_MuMuJJ_BoostedSignalRegion2016.root'    
    elif year == '2017':
        outputFile = workspaceOutputDirectory+'WR_NR_MuMuJJ_BoostedSignalRegion2017.root'
    elif year == '2018':
        outputFile = workspaceOutputDirectory+'WR_NR_MuMuJJ_BoostedSignalRegion2018.root'
...
#        histoDict['QCD_%sDown'%(syst)].Write()
    ...
    histoDict['TT_LSFUp'].Write()
    histoDict['TT_LSFDown'].Write()
...
def FSBWorkspace(sampleNames,samplesLocation,workspaceOutputDirectory, weights, ZPeakSF, doSignal, year):
...
#        if "WRto" in sample: continue
        ...
        if 'TTTo' in sample:
          if 'TTToSemi' in sample:
            ...
            for syst in systs:
                ...
                histoDictFSB['TT_%sUp'%(syst)] = temp.Rebin(Nbins, 'TT_%s_%sUp'%(syst,year), binBoundariesArray)
                ...
                histoDictFSB['TT_%sDown'%(syst)] = temp.Rebin(Nbins, 'TT_%s_%sDown'%(syst,year), binBoundariesArray)
                histoDictFSB['TT_%sDown'%(syst)].SetDirectory(0)
                histoDictFSB['TT_%sDown'%(syst)].Scale(weights[sample])

            histoDictFSB['TT_LSFUp'] = histoDictFSB['TT'].Clone("TT_LSF_%sUp"%(year))
            histoDictFSB['TT_LSFUp'].SetDirectory(0)

            histoDictFSB['TT_LSFUp'].Scale(LSFSFUp/LSFSF)
            histoDictFSB['TT_LSFDown'] = histoDictFSB['TT'].Clone("TT_LSF_%sDown"%(year))
            histoDictFSB['TT_LSFDown'].SetDirectory(0)
            histoDictFSB['TT_LSFDown'].Scale(LSFSFDown/LSFSF)

          else:
            temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass').Rebin(Nbins, 'TT', binBoundariesArray)
            histoDictFSB['TT'].Add(temp,weights[sample])
            histoDictFSB['TT_LSFUp'].Add(temp,weights[sample]*LSFSFUp/LSFSF)
            histoDictFSB['TT_LSFDown'].Add(temp,weights[sample]*LSFSFDown/LSFSF)

            for syst in systs:
	       print "syst: ", syst
               temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sUp'%(syst)).Rebin(Nbins, 'TT_temp', binBoundariesArray)
               histoDictFSB['TT_%sUp'%(syst)].Add(temp,weights[sample])
               temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sDown'%(syst)).Rebin(Nbins, 'TT_temp', binBoundariesArray)
               histoDictFSB['TT_%sDown'%(syst)].Add(temp,weights[sample])
	    ...
	    if 'WW_Tune' in sample:
                ...
                for syst in systs:
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sUp'%(syst))
                    histoDictFSB['DiBoson_%sUp'%(syst)] = temp.Rebin(Nbins, 'DiBoson_%s_%sUp'%(syst,year), binBoundariesArray)
                    histoDictFSB['DiBoson_%sUp'%(syst)].SetDirectory(0)
                    histoDictFSB['DiBoson_%sUp'%(syst)].Scale(weights[sample])
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sDown'%(syst))
                    histoDictFSB['DiBoson_%sDown'%(syst)] = temp.Rebin(Nbins, 'DiBoson_%s_%sDown'%(syst,year), binBoundariesArray)
                    histoDictFSB['DiBoson_%sDown'%(syst)].SetDirectory(0)
                    histoDictFSB['DiBoson_%sDown'%(syst)].Scale(weights[sample])

                histoDictFSB['DiBoson_LSFUp'] = histoDictFSB['DiBoson'].Clone("DiBoson_LSF_%sUp"%(year))
                histoDictFSB['DiBoson_LSFUp'].SetDirectory(0)

                histoDictFSB['DiBoson_LSFUp'].Scale(LSFSFUp/LSFSF)
                histoDictFSB['DiBoson_LSFDown'] = histoDictFSB['DiBoson'].Clone("DiBoson_LSF_%sDown"%(year))
                histoDictFSB['DiBoson_LSFDown'].SetDirectory(0)
                histoDictFSB['DiBoson_LSFDown'].Scale(LSFSFDown/LSFSF)
		...
		temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass').Rebin(Nbins, 'DiBoson_temp', binBoundariesArray)
                ...
                histoDictFSB['DiBoson_LSFUp'].Add(temp,weights[sample]*LSFSFUp/LSFSF)
                histoDictFSB['DiBoson_LSFDown'].Add(temp,weights[sample]*LSFSFDown/LSFSF)
                for syst in systs:
	    ...
	    if 'WWW_4F_Tune' in sample:
		    ...
		    print "syst: ", syst
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sUp'%(syst))
                    histoDictFSB['TriBoson_%sUp'%(syst)] = temp.Rebin(Nbins, 'TriBoson_%s_%sUp'%(syst,year), binBoundariesArray)
                    histoDictFSB['TriBoson_%sUp'%(syst)].SetDirectory(0)
                    histoDictFSB['TriBoson_%sUp'%(syst)].Scale(weights[sample])
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sDown'%(syst))
                    histoDictFSB['TriBoson_%sDown'%(syst)] = temp.Rebin(Nbins, 'TriBoson_%s_%sDown'%(syst,year), binBoundariesArray)
                    histoDictFSB['TriBoson_%sDown'%(syst)].SetDirectory(0)
                    histoDictFSB['TriBoson_%sDown'%(syst)].Scale(weights[sample])

                histoDictFSB['TriBoson_LSFUp'] = histoDictFSB['TriBoson'].Clone("TriBoson_LSF_%sUp"%(year))
                histoDictFSB['TriBoson_LSFUp'].SetDirectory(0)
                histoDictFSB['TriBoson_LSFUp'].Scale(LSFSFUp/LSFSF)
                histoDictFSB['TriBoson_LSFDown'] = histoDictFSB['TriBoson'].Clone("TriBoson_LSF_%sDown"%(year))
                histoDictFSB['TriBoson_LSFDown'].SetDirectory(0)
                histoDictFSB['TriBoson_LSFDown'].Scale(LSFSFDown/LSFSF)
                ...
                histoDictFSB['TriBoson_LSFUp'].Add(temp,weights[sample]*LSFSFUp/LSFSF)
                histoDictFSB['TriBoson_LSFDown'].Add(temp,weights[sample]*LSFSFDown/LSFSF)
                for syst in systs:
            ...
            if 'TTWJetsToLNu' in sample or 'ttWJets' in sample:
                ...
                for syst in systs:
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sUp'%(syst))
                    histoDictFSB['ttV_%sUp'%(syst)] = temp.Rebin(Nbins, 'ttV_%s_%sUp'%(syst,year), binBoundariesArray)
                    histoDictFSB['ttV_%sUp'%(syst)].SetDirectory(0)
                    histoDictFSB['ttV_%sUp'%(syst)].Scale(weights[sample])
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sDown'%(syst))
                    histoDictFSB['ttV_%sDown'%(syst)] = temp.Rebin(Nbins, 'ttV_%s_%sDown'%(syst,year), binBoundariesArray)
                    histoDictFSB['ttV_%sDown'%(syst)].SetDirectory(0)
                    histoDictFSB['ttV_%sDown'%(syst)].Scale(weights[sample])

                histoDictFSB['ttV_LSFUp'] = histoDictFSB['ttV'].Clone("ttV_LSF_%sUp"%(year))
                histoDictFSB['ttV_LSFUp'].SetDirectory(0)
                histoDictFSB['ttV_LSFUp'].Scale(LSFSFUp/LSFSF)
                histoDictFSB['ttV_LSFDown'] = histoDictFSB['ttV'].Clone("ttV_LSF_%sDown"%(year))
                histoDictFSB['ttV_LSFDown'].SetDirectory(0)
                histoDictFSB['ttV_LSFDown'].Scale(LSFSFDown/LSFSF)
            else:
                ...
                histoDictFSB['ttV_LSFUp'].Add(temp,weights[sample]*LSFSFUp/LSFSF)
                histoDictFSB['ttV_LSFDown'].Add(temp,weights[sample]*LSFSFDown/LSFSF)
                for syst in systs:
	    ...
	    if 'ST_s-channel' in sample:
		    ...
		    print "syst: ", syst
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sUp'%(syst))
                    histoDictFSB['ST_%sUp'%(syst)] = temp.Rebin(Nbins, 'ST_%s_%sUp'%(syst,year), binBoundariesArray)
                    histoDictFSB['ST_%sUp'%(syst)].SetDirectory(0)
                    histoDictFSB['ST_%sUp'%(syst)].Scale(weights[sample])
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sDown'%(syst))
                    histoDictFSB['ST_%sDown'%(syst)] = temp.Rebin(Nbins, 'ST_%s_%sDown'%(syst,year), binBoundariesArray)
                    histoDictFSB['ST_%sDown'%(syst)].SetDirectory(0)
                    histoDictFSB['ST_%sDown'%(syst)].Scale(weights[sample])

                histoDictFSB['ST_LSFUp'] = histoDictFSB['ST'].Clone("ST_LSF_%sUp"%(year))
                histoDictFSB['ST_LSFUp'].SetDirectory(0)
                histoDictFSB['ST_LSFUp'].Scale(LSFSFUp/LSFSF)
                histoDictFSB['ST_LSFDown'] = histoDictFSB['ST'].Clone("ST_LSF_%sDown"%(year))
                histoDictFSB['ST_LSFDown'].SetDirectory(0)
                histoDictFSB['ST_LSFDown'].Scale(LSFSFDown/LSFSF)
            else:
		temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass').Rebin(Nbins, 'ST_temp', binBoundariesArray)
                ...
                histoDictFSB['ST_LSFUp'].Add(temp,weights[sample]*LSFSFUp/LSFSF)
                histoDictFSB['ST_LSFDown'].Add(temp,weights[sample]*LSFSFDown/LSFSF)
                for syst in systs:
		...
		temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass')
                ...
                for syst in systs:
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sUp'%(syst))
                    histoDictFSB['WJets_%sUp'%(syst)] = temp.Rebin(Nbins, 'WJets_%s_%sUp'%(syst,year), binBoundariesArray)
                    histoDictFSB['WJets_%sUp'%(syst)].SetDirectory(0)
                    histoDictFSB['WJets_%sUp'%(syst)].Scale(weights[sample])
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sDown'%(syst))
                    histoDictFSB['WJets_%sDown'%(syst)] = temp.Rebin(Nbins, 'WJets_%s_%sDown'%(syst,year), binBoundariesArray)
                    histoDictFSB['WJets_%sDown'%(syst)].SetDirectory(0)
                    histoDictFSB['WJets_%sDown'%(syst)].Scale(weights[sample])

                histoDictFSB['WJets_LSFUp'] = histoDictFSB['WJets'].Clone("WJets_LSF_%sUp"%(year))
                histoDictFSB['WJets_LSFUp'].SetDirectory(0)
                histoDictFSB['WJets_LSFUp'].Scale(LSFSFUp/LSFSF)
                histoDictFSB['WJets_LSFDown'] = histoDictFSB['WJets'].Clone("WJets_LSF_%sDown"%(year))
                histoDictFSB['WJets_LSFDown'].SetDirectory(0)
                histoDictFSB['WJets_LSFDown'].Scale(LSFSFDown/LSFSF)

            else:
                temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass').Rebin(Nbins, 'WJets_temp', binBoundariesArray)
                histoDictFSB['WJets'].Add(temp,weights[sample])
                histoDictFSB['WJets_LSFUp'].Add(temp,weights[sample]*LSFSFUp/LSFSF)
                histoDictFSB['WJets_LSFDown'].Add(temp,weights[sample]*LSFSFDown/LSFSF)
                for syst in systs:
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sUp'%(syst)).Rebin(Nbins, 'WJets_temp', binBoundariesArray)
                    histoDictFSB['WJets_%sUp'%(syst)].Add(temp,weights[sample])
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sDown'%(syst)).Rebin(Nbins, 'WJets_temp', binBoundariesArray)
                    histoDictFSB['WJets_%sDown'%(syst)].Add(temp,weights[sample])
            ...
            if 'DYJetsToLL_M-50_HT-100to200' in sample:
                ...
                for syst in systs:
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sUp'%(syst))
                    histoDictFSB['DY_%sUp'%(syst)] = temp.Rebin(Nbins, 'DY_%s_%sUp'%(syst,year), binBoundariesArray)
                    histoDictFSB['DY_%sUp'%(syst)].SetDirectory(0)
                    histoDictFSB['DY_%sUp'%(syst)].Scale(weights[sample])
                    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sDown'%(syst))
                    histoDictFSB['DY_%sDown'%(syst)] = temp.Rebin(Nbins, 'DY_%s_%sDown'%(syst,year), binBoundariesArray)
                    histoDictFSB['DY_%sDown'%(syst)].SetDirectory(0)
                    histoDictFSB['DY_%sDown'%(syst)].Scale(weights[sample])

                histoDictFSB['DY_LSFUp'] = histoDictFSB['DY'].Clone("DY_LSF_%sUp"%(year))
                histoDictFSB['DY_LSFUp'].SetDirectory(0)
                histoDictFSB['DY_LSFUp'].Scale(LSFSFUp/LSFSF)
                histoDictFSB['DY_LSFDown'] = histoDictFSB['DY'].Clone("DY_LSF_%sDown"%(year))
                histoDictFSB['DY_LSFDown'].SetDirectory(0)
                histoDictFSB['DY_LSFDown'].Scale(LSFSFDown/LSFSF)
                temp = tfile.Get('analysis/eventsFailResPassBoostRECO/leadAK8JetMuonMass_ZweightUp')
                histoDictFSB['DY_ZweightUp'] = temp.Rebin(Nbins, 'DY_Zweight_%sUp'%(year), binBoundariesArray)
                histoDictFSB['DY_ZweightUp'].SetDirectory(0)
                histoDictFSB['DY_ZweightUp'].Scale(weights[sample]*ZPeakSF)
                temp = tfile.Get('analysis/eventsFailResPassBoostRECO/leadAK8JetMuonMass_ZweightDown')
                histoDictFSB['DY_ZweightDown'] = temp.Rebin(Nbins, 'DY_Zweight_%sDown'%(year), binBoundariesArray)
                histoDictFSB['DY_ZweightDown'].SetDirectory(0)
                histoDictFSB['DY_ZweightDown'].Scale(weights[sample]*ZPeakSF)
            else:
		temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass').Rebin(Nbins, 'DY_temp', binBoundariesArray)
                ...
                histoDictFSB['DY_LSFUp'].Add(temp,weights[sample]*LSFSFUp/LSFSF)
                histoDictFSB['DY_LSFDown'].Add(temp,weights[sample]*LSFSFDown/LSFSF)
                for syst in systs:
		    ...
		    temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sDown'%(syst)).Rebin(Nbins, 'DY_temp', binBoundariesArray)
                ...
                temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_ZweightUp').Rebin(Nbins, 'DY_temp', binBoundariesArray)
                histoDictFSB['DY_ZweightUp'].Add(temp,weights[sample]*ZPeakSF)
                temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_ZweightDown').Rebin(Nbins, 'DY_temp', binBoundariesArray)
                histoDictFSB['DY_ZweightDown'].Add(temp,weights[sample]*ZPeakSF)
            ...
            histoDictFSB['WR_%s_NR_%s'%(wrMass,nuMass)].Scale(weightScale)
            temp2 = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass')
            histoDictFSB['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)] = temp2.Rebin(Nbins, 'WR_%s_NR_%s_Numerator'%(wrMass,nuMass), binBoundariesArray)
            histoDictFSB['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].SetDirectory(0)
            histoDictFSB['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].Scale(1/numberOfEvents[sample])
            print "Nominal Numerator: ", histoDictFSB['WR_%s_NR_%s_Numerator'%(wrMass,nuMass)].Integral()
            print "Nominal Weights: ", tfile.Get('analysis/allEvents/m_Scale_muR1_muF1').GetBinContent(1)
            ...
            for syst in systs:
                temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sUp'%(syst))
                histoDictFSB['WR_%s_NR_%s_%sUp'%(wrMass,nuMass,syst)] = temp.Rebin(Nbins, 'WR_%s_NR_%s_%s_%sUp'%(wrMass,nuMass,syst,year), binBoundariesArray)
                histoDictFSB['WR_%s_NR_%s_%sUp'%(wrMass,nuMass,syst)].SetDirectory(0)
                histoDictFSB['WR_%s_NR_%s_%sUp'%(wrMass,nuMass,syst)].Scale(weightScale)
                temp = tfile.Get('analysis/eventsPassBoostFSBRECO/leadAK8JetElectronMass_%sDown'%(syst))
                histoDictFSB['WR_%s_NR_%s_%sDown'%(wrMass,nuMass,syst)] = temp.Rebin(Nbins, 'WR_%s_NR_%s_%s_%sDown'%(wrMass,nuMass,syst,year), binBoundariesArray)
                histoDictFSB['WR_%s_NR_%s_%sDown'%(wrMass,nuMass,syst)].SetDirectory(0)
                histoDictFSB['WR_%s_NR_%s_%sDown'%(wrMass,nuMass,syst)].Scale(weightScale)

            histoDictFSB['WR_%s_NR_%s_LSFUp'%(wrMass,nuMass)] = histoDictFSB['WR_%s_NR_%s'%(wrMass,nuMass)].Clone('WR_%s_NR_%s_LSF_%sUp'%(wrMass,nuMass,year))
            histoDictFSB['WR_%s_NR_%s_LSFUp'%(wrMass,nuMass)].SetDirectory(0)
            histoDictFSB['WR_%s_NR_%s_LSFUp'%(wrMass,nuMass)].Scale(LSFSFUp/LSFSF)
            histoDictFSB['WR_%s_NR_%s_LSFDown'%(wrMass,nuMass)] = histoDictFSB['WR_%s_NR_%s'%(wrMass,nuMass)].Clone('WR_%s_NR_%s_LSF_%sDown'%(wrMass,nuMass,year))
            histoDictFSB['WR_%s_NR_%s_LSFDown'%(wrMass,nuMass)].SetDirectory(0)
            histoDictFSB['WR_%s_NR_%s_LSFDown'%(wrMass,nuMass)].Scale(LSFSFDown/LSFSF)
...
#    outputFile = workspaceOutputDirectory+'WR_NR_MuMuJJ_ResolvedFSB.root'
    if year == '2016':
        outputFile = workspaceOutputDirectory+'WR_NR_MuMuJJ_BoostedFSB2016.root'
    elif year == '2017':
        outputFile = workspaceOutputDirectory+'WR_NR_MuMuJJ_BoostedFSB2017.root'
    elif year == '2018':
        outputFile = workspaceOutputDirectory+'WR_NR_MuMuJJ_BoostedFSB2018.root'
    ...
    histoDictFSB['TT'].Write()
    histoDictFSB['DiBoson'].Write()
    histoDictFSB['TriBoson'].Write()
    histoDictFSB['ttV'].Write()
    histoDictFSB['ST'].Write()
    histoDictFSB['WJets'].Write()
    histoDictFSB['DY'].Write()
    histoDictFSB['data_obs'].Write()
    for syst in systs:
	if syst == "MuL":
	    histoDictFSB['TT_%sUp'%(syst)].SetName("TT_MuID_%sUp"%(year))
            histoDictFSB['TT_%sDown'%(syst)].SetName("TT_MuID_%sDown"%(year))
            histoDictFSB['DiBoson_%sUp'%(syst)].SetName("DiBoson_MuID_%sUp"%(year))
            histoDictFSB['DiBoson_%sDown'%(syst)].SetName("DiBoson_MuID_%sDown"%(year))
            histoDictFSB['TriBoson_%sUp'%(syst)].SetName("TriBoson_MuID_%sUp"%(year))
            histoDictFSB['TriBoson_%sDown'%(syst)].SetName("TriBoson_MuID_%sDown"%(year))
            histoDictFSB['ttV_%sUp'%(syst)].SetName("ttV_MuID_%sUp"%(year))
            histoDictFSB['ttV_%sDown'%(syst)].SetName("ttV_MuID_%sDown"%(year))
            histoDictFSB['ST_%sUp'%(syst)].SetName("ST_MuID_%sUp"%(year))
            histoDictFSB['ST_%sDown'%(syst)].SetName("ST_MuID_%sDown"%(year))
            histoDictFSB['WJets_%sUp'%(syst)].SetName("WJets_MuID_%sUp"%(year))
            histoDictFSB['WJets_%sDown'%(syst)].SetName("WJets_MuID_%sDown"%(year))
            histoDictFSB['DY_%sUp'%(syst)].SetName("DY_MuID_%sUp"%(year))
            histoDictFSB['DY_%sDown'%(syst)].SetName("DY_MuID_%sDown"%(year))

        histoDictFSB['TT_%sUp'%(syst)].Write()
        histoDictFSB['TT_%sDown'%(syst)].Write()
        histoDictFSB['DiBoson_%sUp'%(syst)].Write()
        histoDictFSB['DiBoson_%sDown'%(syst)].Write()
        histoDictFSB['TriBoson_%sUp'%(syst)].Write()
        histoDictFSB['TriBoson_%sDown'%(syst)].Write()
        histoDictFSB['ttV_%sUp'%(syst)].Write()
        histoDictFSB['ttV_%sDown'%(syst)].Write()
        histoDictFSB['ST_%sUp'%(syst)].Write()
        histoDictFSB['ST_%sDown'%(syst)].Write()
        histoDictFSB['WJets_%sUp'%(syst)].Write()
        histoDictFSB['WJets_%sDown'%(syst)].Write()
        histoDictFSB['DY_%sUp'%(syst)].Write()
        histoDictFSB['DY_%sDown'%(syst)].Write()

    histoDictFSB['TriBoson_LSFUp'].Write()
    histoDictFSB['TriBoson_LSFDown'].Write()
    histoDictFSB['DiBoson_LSFUp'].Write()
    histoDictFSB['DiBoson_LSFDown'].Write()
    histoDictFSB['ttV_LSFUp'].Write()
    histoDictFSB['ttV_LSFDown'].Write()
    histoDictFSB['ST_LSFUp'].Write()
    histoDictFSB['ST_LSFDown'].Write()
    histoDictFSB['WJets_LSFUp'].Write()
    histoDictFSB['WJets_LSFDown'].Write()
    histoDictFSB['DY_LSFUp'].Write()
    histoDictFSB['DY_LSFDown'].Write()
    histoDictFSB['DY_ZweightUp'].Write()
    histoDictFSB['DY_ZweightDown'].Write()
    histoDictFSB['TT_LSFUp'].Write()
    histoDictFSB['TT_LSFDown'].Write()
...
#    signalMasses = [(1000,100),(1500,100),(2000,100),(2500,100),(3000,100),(3500,100),(4000,100),(4500,100),(5000,100),(5500,100),(1000,300),(1500,300),(2000,300),(2500,300),(3000,300),(3500,300),(4000,300),(4500,300),(5000,300),(5500,300),(2000,500),(2500,500),(3000,500),(3500,500),(4000,500),(4500,500),(5000,500),(5500,500),(2000,700),(2500,700),(3000,700),(3500,700),(4000,700),(4500,700),(5000,700),(5500,700),(3000,900),(3500,900),(4000,900),(4500,900),(5000,900),(5500,900),(3500,1100),(4000,1100),(4500,1100),(5000,1100),(5500,1100),(4500,1300),(5000,1300),(5500,1300),(4500,1500),(5000,1500),(5500,1500),(5500,1700),(5500,1900),(5000,1000),(4500,450),(4000,800),(4000,400),(3500,350),(3000,600),(2500,250),(2000,400),(2000,200),(1500,150),(1000,200)]
    if(doSignal):
        ...
        for WRmass,NRmass in signalMasses:
            histoDictFSB['WR_%s_NR_%s'%(WRmass,NRmass)].Write()
            histoDictFSB['WR_%s_NR_%s_Numerator'%(WRmass,NRmass)].Write()
            for syst in systs:
		if syst == "MuL":
		    histoDictFSB['WR_%s_NR_%s_%sUp'%(WRmass,NRmass,syst)].SetName("WR_%s_NR_%s_MuID_%sUp"%(WRmass,NRmass,year))
                    histoDictFSB['WR_%s_NR_%s_%sDown'%(WRmass,NRmass,syst)].SetName("WR_%s_NR_%s_MuID_%sDown"%(WRmass,NRmass,year))

                histoDictFSB['WR_%s_NR_%s_%sUp'%(WRmass,NRmass,syst)].Write()
                histoDictFSB['WR_%s_NR_%s_%sDown'%(WRmass,NRmass,syst)].Write()
            histoDictFSB['WR_%s_NR_%s_LSFUp'%(WRmass,NRmass)].Write()
            histoDictFSB['WR_%s_NR_%s_LSFDown'%(WRmass,NRmass)].Write()
...
FSBWorkspace(sampleNames,samplesLocation,workspaceOutputDirectory, weights, ZPeakSF, doSignal, year)
